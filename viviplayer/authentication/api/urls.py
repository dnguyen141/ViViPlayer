...
urlpatterns = [
    ...
    re_path('users/(?P<pk>[0-9]+)/', CustomUserRetrieveUpdateAPI.as_view(), name="rest_user_details_id"),
    re_path('users/(?P<username>[0-9A-Za-z_\\-]+)/', CustomUserRetrieveUpdateAPI.as_view(), name="rest_user_details_username"),
    path('change_password/', CustomModChangePasswordAPI.as_view(), name="rest_change_pwd"),
